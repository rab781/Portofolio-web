...
## 2025-04-09 - [Semantic Lists for Generic Layout Containers]
**Learning:** The `src/components/Experience.tsx` component renders a visual timeline of user experiences. When using generic `<div>` containers for its structure (e.g., Tailwind's `space-y-*`), screen readers lose the semantic context of a list, reducing accessibility for non-visual users navigating sequential data.
**Action:** When replacing native HTML lists (`<ul>`/`<ol>`) with `<div>` containers for layout purposes, explicitly add `role="list"` to the parent container and `role="listitem"` to the child elements to preserve necessary layout semantics for assistive technologies.
