...
## 2025-07-22 - Hoisting Framer Motion Config Objects
**Learning:** When using Framer Motion hooks like `useSpring`, `useInView`, and `useScroll` inside components, defining static configuration objects inline causes a new object/array allocation on every render cycle. While Framer Motion hooks return MotionValues (which prevents full component re-renders on every animation tick), hoisting these static configurations outside the component body (and typing them with explicitly imported types like `SpringOptions`, `UseInViewOptions`, and `UseScrollOptions["offset"]`) is a best practice that ensures referential stability. This prevents unnecessary internal hook re-evaluations and reduces garbage collection pressure during initial mounts or non-scroll re-renders.
**Action:** Always hoist static Framer Motion configuration objects outside the component body to maintain referential equality and avoid unnecessary allocations.
