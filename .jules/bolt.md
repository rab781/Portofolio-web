...
## 2025-03-05 - Hoisting Static Configuration Objects for Framer Motion Hooks
**Learning:** React components containing Framer Motion hooks like `useSpring` and `useTransform` re-evaluate their arguments on every render. Even though Framer Motion hooks return MotionValues (meaning components don't re-render on every scroll tick if the value is used natively in `<motion.div>`), any static configuration objects passed to these hooks (e.g. `{ damping: 30, stiffness: 100 }` or `{ clamp: true }`) are allocated as new objects on every render. If the component re-renders for other reasons (or when initially mounted), these new object allocations create unnecessary garbage collection pressure and can trigger internal re-computations within Framer Motion if it detects a "new" configuration object reference.
**Action:** Always hoist static configuration objects for Framer Motion hooks (like `useSpring` configs, `useScroll` offsets, `useTransform` options) outside of the React component body to ensure stable references and prevent unnecessary allocations.
