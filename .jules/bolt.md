...
## 2024-05-18 - Hoisting IntersectionObserver configs
**Learning:** Instantiating `IntersectionObserver` with an inline configuration object directly inside a `useEffect` hook causes the configuration object to be re-allocated on every re-render (or every time the effect re-runs). This causes unnecessary garbage collection pressure and could potentially lead to the effect re-running more often than necessary if the dependency array is not managed perfectly.
**Action:** Always hoist static configuration objects for `IntersectionObserver` to the module level (outside the component) to ensure referential stability and reduce allocation overhead.
