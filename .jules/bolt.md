...
## 2025-05-27 - Hoisting Framer Motion Config Objects
**Learning:** When using Framer Motion hooks like `useInView` or `useScroll` inside components, defining configuration objects (like `{ margin: "-10px" }`) inline causes a new object allocation on every render tick. This can trigger unnecessary internal hook re-evaluations and increase garbage collection pressure.
**Action:** Always hoist static Framer Motion configuration objects outside the component body or wrap them in `useMemo` to maintain referential equality. For objects passed to `useInView`, explicitly type them with `UseInViewOptions` to satisfy TypeScript.
