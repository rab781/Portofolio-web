...
## 2026-05-20 - Avoiding meaningles IntersectionObserver micro-optimizations\n**Learning:** Hoisting a static `IntersectionObserver` configuration object outside of a component only saves a tiny object allocation when the component renders. Since these components do not render rapidly in animation loops, this micro-optimization has virtually zero measurable performance impact and violates Bolt's rule against meaningless micro-optimizations.\n**Action:** Focus on optimizations with tangible impact (like debouncing high-frequency events or extracting rapidly rendering children) rather than hoisting static objects that only re-allocate infrequently.
