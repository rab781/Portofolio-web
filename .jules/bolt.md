...
## 2025-04-13 - Extracting Framer Motion configuration objects
**Learning:** Passing inline configuration objects or arrays directly to Framer Motion hooks like `useScroll` and `useSpring` creates new object references on every render tick, forcing React to diff them unnecessarily.
**Action:** When using hooks like `useScroll({ offset: [...] })` or `useSpring(..., { stiffness: 100, damping: 30 })`, hoist these configuration options outside of the component body or memoize them to prevent redundant object allocations during renders. Remember to properly type hoisted tuples using `UseScrollOptions["offset"]` or `as const` so TypeScript is happy.
