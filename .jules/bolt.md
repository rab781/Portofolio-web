...
## 2025-03-05 - Hoisting Framer Motion Config Objects
**Learning:** When using Framer Motion hooks like `useSpring` inside components, defining configuration objects (like `{ stiffness: 100, damping: 30 }`) inline causes a new object allocation on every render tick. This can trigger unnecessary internal hook re-evaluations and increase garbage collection pressure during rapid scroll/animation events.
**Action:** Always hoist static Framer Motion configuration objects or arrays outside the component body or wrap them in `useMemo` to maintain referential equality.
