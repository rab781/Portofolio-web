...
## 2025-03-05 - [Hoist Static Object to Avoid Re-allocations]
**Learning:** Defining static objects like `observerOptions` inside a `useEffect` hook causes them to be re-allocated on every execution, increasing garbage collection overhead.
**Action:** Always hoist invariant configuration objects, such as IntersectionObserver options, outside the component function or module scope.
