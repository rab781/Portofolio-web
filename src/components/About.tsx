...
// ⚡ Bolt: Hoisted static configuration objects outside the component body
// to prevent unnecessary object allocations on initial mount or non-scroll re-renders.
const COUNTER_SPRING_CONFIG = { damping: 30, stiffness: 100 };
const COUNTER_IN_VIEW_CONFIG: UseInViewOptions = { once: true, margin: "-10px" };
...
function NumberCounter({ value, suffix = "+" }: { value: number; suffix?: string }) {
  ...
  const isInView = useInView(ref, COUNTER_IN_VIEW_CONFIG);
