...
// to prevent unnecessary object allocations on every render.
const NUMBER_SPRING_CONFIG = { damping: 30, stiffness: 100 };
const NUMBER_IN_VIEW_OPTIONS: UseInViewOptions = { once: true, margin: "-10px" };
...
function NumberCounter({ value, suffix = "+" }: { value: number; suffix?: string }) {
  ...
  const isInView = useInView(ref, NUMBER_IN_VIEW_OPTIONS);
